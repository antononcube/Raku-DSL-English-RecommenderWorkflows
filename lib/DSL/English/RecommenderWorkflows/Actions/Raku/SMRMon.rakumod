...
#==============================================================================
#
#   Raku::SMRMon actions in Raku Perl 6
...
class DSL::English::RecommenderWorkflows::Actions::Raku::SMRMon
  ...
  method load-data($/) { make 'set-data(data = ' ~ $<data-location-spec>.made ~ ')'; }
  ...
  method create-by-dataset($/) {
    with $<colid> {
      ...
      make 'my $obj = ML::SparseMatrixRecommender.new.create-from-wide-form(' ~ $<dataset-name>.made ~ ')';
    }
  }
  method create-by-matrices($/) { make 'my $obj = ML::SparseMatrixRecommender.new.create-from-matrices(' ~ $<variable-names-list>.made ~ ')'; }

  # Data statistics command
  method statistics-command($/) { make $/.values[0].made; }
  method show-data-summary($/) { make '.echo-data-summary()'; }
  method summarize-data($/) { make '.echo-data-summary()'; }
  method items-per-tag($/) { make '.items-per-tag-distribution()'; }
  method tags-per-items($/) { make '.items-per-tag-distribution()'; }
  ...
  method recommend-by-history($/) { make '.recommend(' ~ $<history-spec>.made ~ ')'; }
  method top-recommendations($/) { make '.get-top-recommendations(spec => Whaterver, nrecs => ' ~ $<integer-value>.made ~ ')'; }
  method top-recommendations-by-history($/) { make '.recommend( ' ~ $<history-spec>.made ~ ', ' ~ $<top-recommendations><integer-value>.made ~ ')'; }
  method history-spec($/) { make $/.values[0].made; }

  # Recommend by profile command
  method recommend-by-profile-command($/) { make $/.values[0].made; }
  method recommend-by-profile($/) { make '.recommend-by-profile(' ~ $<profile-spec>.made ~ ')'; }
  method top-profile-recommendations($/) { make '.get-top-recommendations( spec => Whatever, nrecs => ' ~ $<integer-value>.made ~ ')'; }
  method top-recommendations-by-profile($/) { make '.recommend-by-profile(' ~ $<profile-spec>.made ~ ', ' ~ $<top-recommendations><integer-value>.made ~ ')'; }
  ...
  # Make profile
  method make-profile-command($/) { make '.profile(' ~ $<history-spec>.made ~ ')'; }
  ...
  method classify-by-profile($/) {
    if $<ntop-nns> {
      make '.classify-by-profile(' ~ $<tag-type-id>.made ~ ', ' ~ $<profile-spec>.made ~ ', n-top-nearest-neighbors => ' ~ $<ntop-nns>.made ~ ')';
    } else {
      make '.classify-by-profile(' ~ $<tag-type-id>.made ~ ', ' ~ $<profile-spec>.made ~ ')';
    }
  }
  ...
  method plot-recommendation-scores($/) { make '.plot-scores()'; }
  ...
  method smr-context-property-spec($/) { make 'obj = get-property( ' ~ $/.values[0].made ~ '); obj.echo-value;'; }
  ...
  method smr-matrix-property-spec($/) { make '.get-matrix-property( ' ~ $<smr-matrix-property>.made ~ ', tag-type => NULL ); print( SMRMonEchoValue( smrObj = obj ) )'; }
  method smr-sub-matrix-property-spec($/) { make '.get-matrix-property( ' ~ $<smr-matrix-property>.made ~ ', tag-type => ' ~ $<tag-type-id>.made ~ ' ); print( SMRMonEchoValue( smrObj = obj ) )'; }
  method smr-matrix-property($/) { make $/.values[0].made(); }
  method smr-property-id($/) { make '"' ~ $/.Str ~ '"'; }
  method number-of-columns($/) { make '"number-of-columns"'; }
  method number-of-rows($/) { make '"number-of-rows"'; }
  ...
  method smr-filter-matrix($/) { make '.filter-matrix( profile = ' ~ $<profile-spec>.made ~ ')';  }

  # Make metadata recommender command
  method make-metadata-recommender-command($/) { make $/.values[0].made; }
  method make-metadata-recommender-simple($/) { make '.make-tag-type-recommender( tag-type-to => ' ~ $<tag-type-id>.made ~ ' )'; }
  method make-metadata-recommender-full($/) { make '.make-tag-type-recommender( tag-type-to => ' ~ $<tag-type-id>.made ~ ', tag-types => ' ~ $<tag-type-ids-list>.made ~ ' )'; }

  # Recommender algebra command
  method recommender-algebra-command($/) { make $/.values[0].made; }
  method annex-matrix-command($/) {
    if $<tagtype> {
      make '.annex-sub-matrix( {' ~ $<tagtype>.made ~ ' => ' ~ $<mat>.made ~ '} )';
    } else {
      make '.annex-sub-matrix( {NewType => ' ~ $<mat>.made ~ '} )';
    }
  }
  method join-recommenders-command($/) {
    if $<jointype> {
      make '.join(' ~ $<smr>.made ~ ', join-type => '  ~ $<jointype>.made ~ ' )';
    } else {
      make '.join(' ~ $<smr>.made ~ ' )';
    }
  }
  method remove-tag-types-commands($/) { make 'remove-tag-types(' ~ $/.values[0].made ~ ' )'; }
  ...
  method echo-pipeline-context($/) { make '.echo-context()'; }
  ...
  method setup-code-command($/) {
    make 'SETUPCODE' => "use ML::SparseMatrixRecommender;\n";
  }
}
