...
class DSL::English::RecommenderWorkflows::Actions::Raku::SBR
  ...
  method create-by-dataset($/) { make self.create-simple($/) ~ ";\n" ~ '$sbrObj.ingest( data => ' ~ $<dataset-name>.made ~ ')'; }
  method create-by-matrices($/) { make 'sbrObj.ingestMatrices( matrices => ' ~ $<creation-matrices-spec>.made ~ ')'; }
  ...
  method recommend-by-history($/) { make 'sbrObj.recommend( history => ' ~ $<history-spec>.made ~ ')'; }
  ...
  method top-recommendations-by-history($/) {
    if $<top-recommendations><integer-value> {
      make 'sbrObj.recommend( history => ' ~ $<history-spec>.made ~ ', ' ~ $<top-recommendations><integer-value>.made ~ ')';
    } else {
      make 'sbrObj.recommend( history => ' ~ $<history-spec>.made ~ ')';
    }
  }
  ...
  method top-recommendations-by-profile($/) {
    if $<top-recommendations><integer-value> {
      make 'sbrObj.recommendByProfile( profile => ' ~ $<profile-spec>.made ~ ', nrecs => ' ~ $<top-recommendations><integer-value>.made ~ ')';
    } else {
      make 'sbrObj.recommendByProfile( profile => ' ~ $<profile-spec>.made ~ ')';
    }
  }
  method profile-spec($/) { make $/.values[0].made; }

  # Make profile
  method make-profile-command($/) { make 'sbrObj.profile( history => ' ~ $<history-spec>.made ~ ')'; }
  ...
  method extend-recommendations-simple-command($/) {
    if $<extension-data-id-column-spec> {
      make 'sbrObj.joinAcross( data => ' ~ $<dataset-name>.made ~ ', by => ' ~ $<extension-data-id-column-spec>.made ~ ' )';
    } else {
      make 'sbrObj.joinAcross( data => ' ~ $<dataset-name>.made ~ ' )';
    }
  }
  ...
  method smr-context-property-spec($/) { make 'smrObh.getProperty(' ~ $/.values[0].made ~ ')'; }
  ...
  method smr-filter-matrix($/) { make 'sbrObj.filterMatrix( profile => ' ~ $<profile-spec>.made ~ ')';  }
  ...
  method annex-matrix-command($/) {
    if $<tagtype> {
      make 'sbrObj.annexSubMatrix( newSubMat => ' ~ $<mat>.made ~ ', newTagType => ' ~ $<tagtype>.made ~ ' )';
    } else {
      make 'sbrObj.annexSubMatrix( newSubMat => ' ~ $<mat>.made ~ ', newTagType => "NewType" )';
    }
  }
  method join-recommenders-command($/) {
    if $<jointype> {
      make 'sbrObj.join( smr2 => ' ~ $<smr>.made ~ ', joinType => '  ~ $<jointype>.made ~ ' )';
    } else {
      make 'sbrObj.join( smr2 => ' ~ $<smr>.made ~ ' )';
    }
  }
  method remove-tag-types-commands($/) { make 'sbrObj.removeTagTypes( removeTagTypes => ' ~ $/.values[0].made ~ ' )'; }
  ...
  ## Value
  method assign-pipeline-value-to($/) { make $/.values[0].made ~ ' = sbrObj.getValue()';  }
  method take-pipeline-value($/) { make 'sbrObj.takeValue()'; }
  method echo-pipeline-value($/) { make 'sbrObj.takeValue()'; }
  method echo-pipeline-funciton-value($/) { make 'sbrObj.applyFunctionToValue( ' ~ $<pipeline-function-spec>.made ~ ' )'; }
